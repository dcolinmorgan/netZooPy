predScore(indir='data/MotifPipeline/sthlm_motif_5_QCbeta/red',outdir='data/MotifPipeline/sthlm_motif_5_QCbeta/red/test',agg_meth='mean',cell=None,TF=None,region=None,TSS=None,depth=None)
...
def predScore(indir='data/MotifPipeline/sthlm_motif_5_QCbeta/red',outdir='data/MotifPipeline/sthlm_motif_5_QCbeta/red/test',agg_meth='mean',cell=None,TF=None,region=None,TSS=None,depth=None):
    ...
    if TSS is not None:
        RR='all'
        DD='all'
        Path(outdir+'/'+TSS).mkdir(parents=True, exist_ok=True)
    ...
    for jac,trace in enumerate(traces):
        ...
        try:
            ...
            if (data.shape[1])>15:
                ...
                # if data.W1 is not int:
                    # try:
                data.W1=data.W1.astype('int')
                    # data=pd.read_csv(trace,sep='\t',usecols=[0,1,2,3,4,8,9,10,11,15,16],names=["chr", "start", "end",'weight','pval',"depth",'W1','array','region','gene','ChIPTF'])
            ...
            #     data=pd.read_csv(trace,sep='\t',usecols=[0,1,2,3,8,9,10],names=["chr", "start", "end",'weight',"W1",'array','ChIPTF'])
            elif(data.shape[1])<10 and TSS is None:
            ...
            elif(data.shape[1])<10 and TSS is not None:
                data=pd.read_csv(trace,sep='\t',usecols=[0,1,2,3,4,8,9,10,11,12,13],names=["chr", "start", "end",'weight','CG','hits',"W1",'array','ChIPTF','body','TSS'])
    ...
    #             table.append(column)
                ...
                if TSS is none:
                    ...
                    np.transpose(pd.DataFrame((column))).to_csv(outdir+'/'+TSS+'/sthlm_PRE_overall.txt',mode='a')#,header=['cell','tf','mauroc','wauroc','meauroc','maupr','waupr','meaupr','size','max_hit','mo_length'])
            ...
            if TSS is none:
                ...
                np.transpose(pd.DataFrame((column))).to_csv(outdir+'/'+TSS+'/sthlm_PRE_overall.txt',mode='a')
        if cell is not None:
            plt.show()
            break
    return RR, DD
